...
FIXED_EVENTS = [
    ...
    (4, 12, "День космонавтики", "low", "+0-5%", "Обычно слабое влияние, но возможны корпоративные заказы."),
    (4, 30, "Родительский день / Радоница", "medium", "+5-20% для гвоздик и ритуальных цветов", "Дата плавающая; фиксированная дата здесь только напоминание проверить православный календарь."),
    (5, 1, "Праздник Весны и Труда", "low", "+0-10%", "Длинные выходные могут менять поток покупателей."),
    (5, 9, "День Победы", "high", "+5-25% для гвоздик", "Заметно влияет на гвоздики: цветы покупают для возложения к памятникам и мемориалам."),
    (5, 12, "День медицинской сестры", "medium", "+5-15%", "Подарочные букеты для медработников; влияние зависит от локальных учреждений."),
    (6, 1, "День защиты детей", "low", "+0-10%", "Небольшой рост подарочных покупок."),
    (6, 12, "День России", "low", "+0-10%", "Праздничные выходные могут менять посещаемость точек."),
    (6, 21, "День медицинского работника", "medium", "+5-20%", "Профпраздник с возможным ростом подарочных букетов."),
    (7, 8, "День семьи, любви и верности", "medium", "+5-15%", "Умеренный спрос на подарочные букеты."),
    (8, 1, "Свадебный сезон: август", "medium", "+5-20%", "Август часто дает спрос на розы, кустовые цветы и композиции."),
    ...
    (10, 19, "День отца", "low", "+0-10%", "Плавающая дата в октябре; напоминание проверить календарь."),
    (11, 4, "День народного единства", "low", "+0-10%", "Выходной день может менять трафик магазинов."),
...
def demand_events_for_period(start: date, end: date) -> list[DemandEvent]:
    ...
    for year in range(lookup_start.year, lookup_end.year + 1):
        ...
        for approximate_day, name, impact, adjustment, note in [
            ...
            (
                date(year, 12, 20),
                "Корпоративные новогодние поздравления",
                "medium",
                "+5-20%",
                "Корпоративные заказы часто появляются заранее, до основных новогодних выходных.",
            ),
        ]:
