...
Что еще нужно добавить для следующего этапа:

1. Авторизация в приложении.
2. Админ-раздел.
3. Смена пароля администратора.
4. Просмотр списка backup-файлов.
5. Восстановление базы из backup.
6. Журнал действий администратора.
7. Защита опасных действий, например `Очистить базу`, отдельным подтверждением.

Для пилота лучше делать это внутри FastAPI, а не только Basic Auth в nginx, потому что нам нужны операции с базой и backup-ами из интерфейса.

## Следующий этап: простая админ-панель

Предлагаемый минимальный состав:

- `/login`;
- `/logout`;
- cookie-сессия;
- хранение хэша пароля в SQLite;
- страница `/admin`;
- смена пароля;
...
- лог действий;
- защита `Очистить базу` авторизацией и подтверждением.

Такой вариант закрывает не только вход по паролю, но и нормальную эксплуатацию пилота без SSH.
